...
App app = { initObject(), 'X', {0}, 0, 0, 0, 0, 0, 0, 0};
...
void transfer_enable(Task_schedule*, int);
...
void play(Task_schedule *self, int flag){
	if(flag % 2){
		self->enable = 1;
		ASYNC(&task_schedule, transfer_enable, self->enable);
...
void transfer_enable(Task_schedule *self, int enable){
	int i = 0;
	char buffer[6] = {0};
	CANMsg msg1;
	msg1.msgId = 1;
